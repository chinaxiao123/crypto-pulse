...
def generate_daily_report(coins: list[str], persona: str = "default") -> Optional[str]:
    """
    Generate a daily market report in Markdown.

    Args:
        coins: List of coin symbols to include (e.g. ['BTC', 'ETH', 'BNB'])
...
def _generate_default_report(data: dict, timestamp: str) -> str:
    ...
    lines = []
    lines.append(f"# 📊 Crypto Market Report")
    ...
    for symbol, info in data.items():
        ...
        lines.append(
            ...
            f"{format_volume(info['volume_24h_usd'])} | {format_volume(info['market_cap_usd'])} | "
    ...
    lines.append("---")
    lines.append(f"*Powered by [CryptoPulse](https://github.com/chinaxiao123/crypto-pulse) 💓*")
    return "\n".join(lines)
...
def _generate_rose_report(data: dict, timestamp: str) -> str:
    ...
    """
    lines = []
    lines.append("兄弟们，重仓猛干！🌹")
    lines.append("")
    lines.append(f"> {timestamp} 行情速报，情报员八手玫瑰为您播报")
    lines.append("")
    ...
    lines.append("*Powered by CryptoPulse 💓 + 八手玫瑰 🌹*")
    return "\n".join(lines)
